    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-black/25 text-sm">Loading messages…</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto bg-white px-4 py-4 space-y-0.5">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center py-20">
            ...
            <p className="text-black/30 text-sm">No messages yet — say hello!</p>
          </div>
        )}
          ...
          if (item.type === "date") {
            return (
              <div key={item.key} className="flex items-center gap-3 py-3">
                ...
                <span className="text-[11px] text-black/20 font-medium px-2">
                  {item.label}
                </span>
                <div className="flex-1 h-px bg-white/5" />
              </div>
